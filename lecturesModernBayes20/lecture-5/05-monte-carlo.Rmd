...
- The notes have been updated to hopefully make the rejection sampling material more clear from last time
and I will highlight some of these points today in class. 
...
- You will be given a cover page with distributions.
- This exam will be released over Gradescope at the start of class 10:05 AM and you must complete the assignment no later than 11:20 AM. 
...
- You must upload a single PDF file to Gradescope and label your pages.
- You can write your solutions on your own paper, a tablet, etc. 
- You are not required (or expected) to type your solutions. 
- You may utilize software to help pull your written solutions into a single PDF for uploading purposes. 
- If you use AI software, you must cite this in your write up (or this violates the exam policy). 
- You are not allowed to search for solutions to the problems (or utilize any prior solutions that have been written). 

Quiz I
===

- On this exam, you will not be expected to do any coding/programming, however you may be asked to provide interpretations beyond derivations. 
...
\textbf{Example}: Suppose $h(x)=x$ for all $x.$
=======
...
Y <- rexp(nsim)
par(mfrow=(c(1,2)))
hist(X, freq=FALSE, main="Exponenial via inverse CDF method")
hist(Y, freq=FALSE, main="Exponenial distn directly")
```
...
$$c \textcolor{blue}{q(x)} \geq \tilde{f}(x).$$
\item Sample $X \sim \textcolor{blue}{q}$, sample $Y \sim \text{Unif}(0, c\; \textcolor{blue}{q(X)})$ (given X)
Intuition: We draw uniform samples $(X,Y)$ that fall under the curve \textcolor{blue}{cq.}
\item If $Y \leq \tilde{f}(X), Z=X,$ \textcolor{blue}{otherwise we reject and return to step (2)}. 
Intuition: We keep the points that fall below $\tilde(f).$
%Generate $X \sim g,$ and calculate $r(X) = \dfrac{\ell(X)}{M\; g(X)}.$
%\item Flip a coin with probability of success $r(X).$ If we have a success,
%retain X. Else return to (1).
\end{enumerate}
Output: $Z \sim f(x)$
Intuition: If we accept, The output of the algorithm is the projection onto the 
x-axis of accepted points (and the y-axis is used only for acceptance or rejection). Note that our x samples are random samples from f. 
...
<!-- \] -->
<!-- where $f(x_{1:n} \mid \theta)$ is the likelihood and $\pi(\theta)$ is the prior.  Assume we can calculate $\hat{\theta}_{MLE}$. -->

<!-- \begin{enumerate} -->
<!--   \item Write a rejection sampler that samples points from the posterior distribution.  -->
<!--   \item Explain why this your sampler works.  -->
<!--   \item Remark on a major limitation/challenge of rejection sampling here. -->
<!-- \end{enumerate} -->
