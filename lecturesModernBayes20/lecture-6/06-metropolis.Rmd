...
This is useful as $$\frac{1}{n}\sum_{i=1}^n f(x_i) \approx E_\pi[f(X)].$$
...
\item Sample $\textcolor{red}{u}$ from Uniform$(0,1).$
\item Let $$r = \frac{\tilde{\pi}(x^{*})}{\tilde{\pi}(x_i)}.$$ If $u < r$, accept and $x_{i+1} = x^{*}.$ \textcolor{red}{accept the new data point}
...
theta<-0 ; delta<-1 ; S<-1000 ; THETA<-NULL
