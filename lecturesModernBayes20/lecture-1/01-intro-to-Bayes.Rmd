...
# Define several Beta distributions
params <- list(
  c(1, 1),   # Uniform
  c(2, 2),   # Mild concentration
  c(5, 5),   # Moderate concentration
  c(20, 20)  # Strong concentration
)

# Create a data frame of densities
df <- do.call(rbind, lapply(params, function(p) {
  data.frame(
    x = x,
    density = dbeta(x, p[1], p[2]),
    label = paste0("Beta(", p[1], ", ", p[2], ")")
  )
}))

# Plot
ggplot(df, aes(x, density, color = label)) +
  geom_line(size = 1.2) +
  labs(title = "Symmetric Beta Distributions Centered at 0.5",
       x = "theta", y = "Density") +
...
# Define skewed Beta distributions (towards 1)
params <- list(
  c(5, 2),    # moderate skew towards 1
  c(10, 3),   # stronger skew
  c(20, 5),   # sharper peak near 1
  c(50, 10)   # very tight near 1
)
...
ggplot(df, aes(x, density, color = label)) +
  geom_line(size = 1.2) +
  labs(title = "Beta Distributions Skewed Towards 1",
       x = "theta", y = "Density") +
  theme_minimal(base_size = 14)
```
...
- The national approval rating (Zogby poll) of President Obama in mid-September 2010 was 50\%. We'll assume that in PA his approval rating is also 50\%. \textcolor{red}{Prior}
...
===
- At times, we may wish to find the \textcolor{red}{conditional distribution of $x_{n+1}$ given $x_{1:n}.$}
