...
- Please take a few minutes to fill out how the course is going and any improvements

https://forms.gle/BXd36Zi37kM4rWsY8

- The exams are still being finalized after we all have done a first pass at grading them. 
- Thank you for your patience as we work to get these back to you as quickly as possible. 
...
===
\textcolor{blue}{Under some conditions, assume that $E|(X,Y)| < \infty.$ Assume that $$(X,Y) \sim p(x,y)$$}

\textcolor{blue}{Then $$\frac{1}{M} \sum_{i=1}^M (X_i, Y_i) \rightarrow E[(X,Y)]$$ as $M \rightarrow \infty.$}

\textcolor{blue}{Thus, we have an approximation of $E[(X,Y)],$ which is referred to MCMC in this setting (the pairs are not i.i.d.)}
...
res <- sampleGibbs(0.25,0.25,n.sims,data)
head(res)
```
...
\begin{enumerate}
    \item First, sample $\textcolor{blue}{U}\sim \Uniform(0,F(c|\theta))$ where $$F(x|\theta) = 1-e^{-\theta x}$$ is the $\Exp(\theta)$ c.d.f.
...
For any fixed value of $\mu$, it is straightforward to derive\footnote{derivation is found at the end of slides} that
...
\begin{align}
X_1, \ldots, X_n \mid \lambda &\stackrel{iid}{\sim} \text{Normal}(\mu, \lambda^{-1})\\
\lambda &\sim \text{Gamma}(a,b)
\end{align}

Let's derive the posterior update of $\lambda \mid x_{1:n},$ which we utilized in an earlier derivation. 

Exercise
===

\begin{align}
p(\lambda \mid x_{1:n}) &\propto p(x_{1:n} \mid \lambda) p(\lambda) \\
&= \prod_{i=1}^n [\sqrt{\frac{\lambda}{2\pi}} 
\exp\{ 
-\frac{\lambda}{2} 
(x_i - \mu)^2
\} ] \times
\frac{b^a}{\Gamma(a)}\lambda^{a-1}e^{-b\lambda}\\
& \propto (\lambda)^{n/2} \exp\{ 
-\frac{\lambda}{2} 
\sum_{i=1}^n (x_i - \mu)^2
\} ] \times \lambda^{a-1}e^{-b\lambda} \\
& \propto (\lambda)^{a + n/2 - 1} 
\exp\{ 
\lambda[b +
\sum_{i=1}^n (x_i - \mu)^2]
\} 
...
\begin{align}
\lambda \mid x_{1:n} \sim \text{Gamma}(a + n/2, b +
\sum_{i=1}^n (x_i - \mu)^2)
\end{align}
